...

{
    ...
    WAVEOUTCAPS tmpcaps;
    waveOutGetDevCaps((UINT_PTR)waveHandle, &tmpcaps, sizeof(WAVEOUTCAPS));
    if (tmpcaps.dwSupport & WAVECAPS_VOLUME)
    {
        supportsVolume = true;
    }
    else
...
bool Audio_MMSystem::open(AudioConfig &cfg)
{
    ...
    // Rev 1.4 (ihp) - Calculate buffer to hold 187,5 ms of data
    bufSize = (wfm.nSamplesPerSec * wfm.nBlockAlign) / 6 ;
...
bool Audio_MMSystem::write()
{
    ...
    if (supportsVolume)
    {
        checkResult(waveOutSetVolume(waveHandle, _vol));
    }
...
void Audio_MMSystem::setVolume(DWORD vol)
{
    _vol = vol;
}
#endif // HAVE_MMSYSTEM
